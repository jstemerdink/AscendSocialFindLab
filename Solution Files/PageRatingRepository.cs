    ...
    public class PageRatingRepository : IPageRatingRepository
    {
        ...
        public Dictionary<string, int> GetFavoriteCategoryNamesForUser(string userId)
        {
            ...
            foreach (ICategorizable page in topRated.OfType<ICategorizable>())
            {
                foreach (int categoryId in page.Category)
                {
                    string categoryName = this.categoryRepository.Get(categoryId).Name;
                    if (favoriteCategories.ContainsKey(categoryName))
                    {
                        favoriteCategories[categoryName] += 1;
                    }
                    else
                    {
                        favoriteCategories.Add(categoryName, 1);
                    }
                }
            }
